...
module.exports = {
  getProperty: ({type, id, props}, getItems) => (game, select, thisObj, targetObj) => {
  ...
  setProperty: ({type, id, props}, value, setItems) => (game, select, thisObj, targetObj) => {
    // console.log(thisObj);
  ...
  addItem: (object, item) => (game, select, thisObj, targetObj) => {
  ...
  addExit: (location, destination) => (game, select, thisObj, targetObj) => {
    return addExitToObject(game, location.id, destination.id);
  },
  removeItem: (object, item) => (game, select, thisObj, targetObj) => {
    const id = object.type === '$INVENTORY' ? 'PLAYER'       :
