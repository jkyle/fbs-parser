...
const buildState = definition => {
  ...
  const exe = definition.reduce((acc, item) => ({...acc, [item.id]: item.events }), {})
...
const writeFiles = (state, exe) => Promise.all([
  fs.writeFileAsync(path.resolve(`${dir}/${(program.output || 'build')}/state.json`), JSON.stringify(state)),
  fs.writeFileAsync(path.resolve(`${dir}/${(program.output || 'build')}/program.json`), JSON.stringify(exe))
])
...
readDir(path.resolve(`${dir}/${(program.input || 'game-objects')}`))
  ...
  .then(parseFiles)
  .then(buildState)
  .then(({state, exe}) => writeFiles(state, exe));